            ...
            {visible.map((product) => {
              ...
              return (
                <tr onClick={() => props.onOpenProduct(product.id)} className={props.selectedIds.has(product.id) ? "selected-row" : ""} key={product.id}>
                  ...
                  <td><div className="source-badges">{product.sources.map((sourceItem) => <span className={sourceItem.sourceKey} title={SOURCE_LABELS[sourceItem.sourceKey]?.label} key={sourceItem.id}>{SOURCE_LABELS[sourceItem.sourceKey]?.short}</span>)}</div></td>
